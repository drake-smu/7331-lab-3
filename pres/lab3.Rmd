...
```{r}
results %>% arrange(desc(avlift)) %>% filter(nrules <=25) %>% filter(nrules >= 20) %>% filter(support > 0.2) %>% filter(conf > 0.2)
```

Looks like we have a clear winner. We will now try the apriori algorithm using our parameters

```{r}
optimalRules <- apriori(data, 
                   parameter = list(conf=0.24, supp = 0.21),
                   appearance = list(
                     rhs=c("income_bracket=small", "income_bracket=large"), 
                     default="lhs"),
                   control = list(verbose=F)) 
redund <- is.redundant(optimalRules)
optimalRules <- optimalRules[redund == F]
